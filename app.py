...
def main():
    ...
    if 'Report_Date' in df.columns:
        # Internal date handling as date objects for strict comparison
        df_dates = pd.to_datetime(df['Report_Date'], errors='coerce').dt.date
        # Default to the latest available snapshot
        selected_date = st.sidebar.date_input("Select Report Date", value=df_dates.max())
        # Strict equality filter for snapshot-only reporting
