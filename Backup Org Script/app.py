...
def main():
    ...
    timeline_options = ["All Time", "Today", "Last Week", "Last Month", "Last Quarter"]
    selected_timeline = st.sidebar.selectbox("Select Period", timeline_options)
    
    # Apply timeline filter (if date column exists)
    df_filtered = df.copy()
    if 'Report_Date' in df_filtered.columns:
        today = datetime.now()
        if selected_timeline == "Today":
            df_filtered = df_filtered[df_filtered['Report_Date'] == today.date()]
        elif selected_timeline == "Last Week":
            week_ago = today - timedelta(days=7)
            df_filtered = df_filtered[df_filtered['Report_Date'] >= week_ago.date()]
        elif selected_timeline == "Last Month":
            month_ago = today - timedelta(days=30)
            df_filtered = df_filtered[df_filtered['Report_Date'] >= month_ago.date()]
        elif selected_timeline == "Last Quarter":
            quarter_ago = today - timedelta(days=90)
            df_filtered = df_filtered[df_filtered['Report_Date'] >= quarter_ago.date()]
